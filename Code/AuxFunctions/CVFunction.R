...
cross.val <- function( .data.train
  ...
  pred.function  <- function(model.obj,prediction=FALSE,outcome=1){
    ...
    rmse <- sqrt(mean((model.preds-model.obj$data.test[,outcome])^2)) #calculate rmse
    if(prediction)
    {
  ...
  model.wrapper <- function(inputs,.data.folds=data.folds,mod.name=.modelName,mod.fun=.model,form=.formula,pred=FALSE){ 
    ...
    pred.res <- pred.function(model.obj.res,pred,outcome.index)
    if(pred){
