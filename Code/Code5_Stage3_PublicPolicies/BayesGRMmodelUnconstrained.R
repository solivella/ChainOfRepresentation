BayesGRMuncons <- "model {
  ...
  for (j in 1:9){
    alpha[j] ~ dnorm(m.alpha, pr.alpha) # Parameters are unconstrained
  }
  alpha[10] ~ dnorm(m.alpha, pr.alpha)T(0,) # Only these two parameters are constrained
  alpha[11] ~ dnorm(m.alpha, pr.alpha)T(0,) # Only these two parameters are constrained
  for (j in 12:J){
    alpha[j] ~ dnorm(m.alpha, pr.alpha) # Parameters are unconstrained
  }
  for (j in 1:J){
